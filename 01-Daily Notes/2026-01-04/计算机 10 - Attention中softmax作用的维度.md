...
且 $softmax$ 是针对已有分数进行的归一化,这里出现的 " 已有分数 " 就不应该有那些改为 0 的分数了,所以置零操作要在进入 `softmax` 之前,考虑到 $softmax$ 是计算 $e^{score}$,改为 0 的分数应该是 $e^{score}=0$,也就是分数不能是 $0$,而是 $-\infty$,才有$e^{-\infty}\approx$
