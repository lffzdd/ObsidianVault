...
# 单摆的势能
单摆的势能：![[attachments/Pasted image 20240627012858.png]] 这并不直观，但若代入 $\cos(\theta)\approx1-\frac{\theta^{2}}{2}$: ![[attachments/Pasted image 20240627013420.png]]
把 $1-\frac{\theta^{2}}{2}$ 的图像和 $\cos(\theta)$ 并排放的话，![[attachments/Pasted image 20240627013548.png]]
# 用多项式逼近

> [!note]- 用级数去逼近
> ![[attachments/10 - 泰勒级数_哔哩哔哩_bilibili 和另外 3 个页面 - 个人 - Microsoft​ Edge 2024-06-27 01-41-03.mp4]]

个中缘由就在于，多项式可比别的函数要友好得多，它又好计算又好求导还好积分，简直是各种好
现在，在 $x=0$ 处用二次多项式来近似它 ![[attachments/Pasted image 20240627015734.png]]
...
$$ 再让 $x=0$ 时函数的切线斜率相同，由此得 $c_1=0$ ![[attachments/Pasted image 20240627020458.png]]
再让二阶导数相同，得 $c_{2}=-\frac{1}{2}$
最终我们得到近似函数 $P(x)=1+0x+(-\frac{1}{2})x^2$
去验证：![[attachments/Pasted image 20240627020905.png]]
$c_0$ 负责让多项式在 $x=0$ 的值和 $\cos(0)$ 相一致，$c_1$ 负责一阶导数相一致，$c_2$ 负责二阶导数
![[attachments/Pasted image 20240627021440.png]]
则
$$
n!\cdot c_{n}=f^{(n)}(x)
$$
...
如果是别的点，例如 $x=\pi$：![[attachments/Pasted image 20240627022059.png]]
...
# 用几何来解释泰勒多项式的二次项
如果 $dx$ 不接近于 $0$，面积的变化想近似得更准确的话，就得考虑误差面积：![[attachments/Pasted image 20240627122342.png]]
...
![[attachments/Pasted image 20240627140120.png]]
...
对 $e^x$，不管 $x$ 取何值，最终结果都会收敛到 $e^x$: ![[attachments/Pasted image 20240627141942.png]]
...
发散：![[attachments/Pasted image 20240627142615.png]]
泰勒级数的收敛半径，能够让多项式的和收敛的最大取值范围：![[attachments/Pasted image 20240627142724.png]]
关于泰勒级数，还有很多可以学的
