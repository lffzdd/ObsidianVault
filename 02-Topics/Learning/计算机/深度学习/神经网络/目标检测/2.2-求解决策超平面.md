...
1. 通过合适的维度转换函数，将低维数据进行升维：![[attachments/Pasted image 20240827202432.png]]
2. 在高维度下求解 SVM 模型+：![[attachments/Pasted image 20240827202500.png]]
有一种方法，能够避免将数据送入高维度进行计算，又能获得同样的分类效果：
![[attachments/Pasted image 20240827202655.png]]
# 推导
![[attachments/Pasted image 20240827203218.png]]
 ![[attachments/Pasted image 20240827203421.png]]
 说明 $W$ 和决策超平面是互相垂直的，同时还满足 $\vec{w}\cdot(\vec{x_m}-\vec{x_n})=2$
  ![[attachments/Pasted image 20240827204043.png]]
  ![[attachments/Pasted image 20240827204138.png]]
   ![[attachments/Pasted image 20240827204310.png]]
   这等同于求解在约束条件下, 向量 $w$ 长度的最小化问题
   ![[attachments/Pasted image 20240827204541.png]]
   ![[attachments/Pasted image 20240827204646.png]]
...
![[attachments/Pasted image 20240828140457.png]]
...
当不满足约束条件时，括号内的值小于 0，（即 $y_{i}*(\vec{W}\cdot\vec{v}_{i}+b)<1$ ，$\vec{v_{i}}$ 在正负超平面之间）：![[attachments/Pasted image 20240830173759.png]]
...
![[attachments/Pasted image 20240830174742.png]]
![[attachments/Pasted image 20240830174759.png]]
+这是违反常理的
![[attachments/Pasted image 20240830174838.png]]
- 如果点不在正负超平面上，$\lambda_{i}=0$
- 如果点在正负超平面上，$\lambda_{i}\neq0$
- 如果点在正负超平面之间，即违反了决策，则 $\lambda_{i}\ge0$
## 可视化约束条件的影响
若只有一个约束条件
![[attachments/Pasted image 20240830233750.png]]
此时在切点处 $f(W)$ 的梯度向量指向其函数值增加最大的方向
![[attachments/Pasted image 20240830234030.png]]
![[attachments/Pasted image 20240830235208.png]] 
...
## 转换为对偶问题
现在，已经有 5 个条件了
![[attachments/Pasted image 20240831163532.png]]
不过在 SVM 中，为了求解的效率和更方便的应用核技巧（Kernel Trick），我们往往会将原问题转换为其自身的对偶问题，再求解+ ![[attachments/Pasted image 20240831163649.png]]
原方程有 $p$ 项，新方程没了
![[attachments/Pasted image 20240831164435.png]]
+同时 $f(W*)$ 是我们的极小值解，所以我们有
$$
q(\lambda_i)\leq f(\vec{w^*})-\sum_{i=1}^s\lambda_i*g_i(\vec{w^*},b^*)\leq f(\vec{w^*})\leq f(w)
$$
![[attachments/Pasted image 20240831165103.png]]
+那么我们可以去寻找一个最优下界 $q(\lambda_{i}^{*})$ （即 $q(\lambda_{i})$ 的最大值），让 $q(\lambda_{i}^{*})$ 与 $f(w^{*})$ 尽可能的接近+
![[attachments/Pasted image 20240831165520.png]]
这就是SVM原问题的对偶问题
![[attachments/Pasted image 20240831165637.png]]
当满足某些特定条件(Slater条件)，比如在当前仿射函数约束下求解凸优化问题时，强对偶成立。
而且我们可以证明当 $q(\lambda_{i}^{*})=f(w^{*})$ 时, 两者同时得到最优解 ![[attachments/Pasted image 20240831165959.png]]
![[attachments/Pasted image 20240831170116.png]]
...
```
![[attachments/Pasted image 20240831172034.png]]
我们把不同 $\lambda$ 下的拉格朗日方程的最小值轨迹画出来，就得到了图中的曲线（下图黄色曲线） ![[attachments/Pasted image 20240831172127.png]]
这个最小值轨迹落在原问题极值下方，说明 $q(\lambda)$ 是原问题的下界 ![[attachments/Pasted image 20240831172556.png]]
我们对 $q(\lambda)$ 求最大值，得到 ![[attachments/Pasted image 20240831172701.png]]
![[attachments/Pasted image 20240831172910.png]]
很多时候对偶问题求解起来更方便，并且能表现出一些非常有用的特征
## 求解对偶问题
回到之前的问题，
![[attachments/Pasted image 20240831173028.png]]
![[attachments/Pasted image 20240831173057.png]]
$$
...
$$
联系之前的图片 ![[attachments/Pasted image 20240831081800.png]]
$\triangledown f=\lambda_{1}\triangledown g_{1}+\lambda_{2}\triangledown g_{2}=\lambda_{1}\vec{x_{1}}+\lambda_{2}\vec{x_{2}}$，结论是一致的
而 $\sum_{i=1}^{s}\lambda_{i}y_{i}=0$, 如图 ![[attachments/Pasted image 20240831225749.png]]
...
\end{align}
$$
![[attachments/Pasted image 20240831232243.png]]
之所以我们绕了这样一大圈去求对偶问题,除了对偶问题从表达式和约束条件上更简洁以外,更主要的原因是因为对偶问题有一个非常好的特征,也就是最优解仅根据支持向量的点积结果所决定
![[attachments/Pasted image 20240831232438.png]]
推导点积：
由于
$$
\begin{align}
...
![[attachments/Pasted image 20240831232253.png]]
...
![[attachments/Pasted image 20240901012829.png]]
