...
前二十五分钟描述了一些历史
## 代码编译过程
![[attachments/Pasted image 20250117140234.png]]
## `mov`
### Src 和 Dest
![[attachments/Pasted image 20250117152818.png]]
出于硬件设计原因, 不能从 memory 移动到 memory (看看后面的存储器结构就知道了)
![[attachments/Pasted image 20250117153035.png]]
当你将寄存器的名称放在括号中时, 就是将寄存器中的值作为地址
### 函数参数寄存器
![[attachments/Pasted image 20250117153449.png]]
事实证明、使用（64）x86-64、函数参数总是出现在某些特定的寄存器中
`rdi`: 第一个
`rsi`: 第二个
![[attachments/Pasted image 20250117155840.png]]

### 寻址指令
![[attachments/Pasted image 20250117154203.png]]
...
![[attachments/Pasted image 20250117154557.png]]
有以上这些精简形式
### `lea`
一个让你困惑的指令
![[attachments/Pasted image 20250117155020.png]]
**它的目标是利用 C 的 & 符号操作来计算地址**, 当也是一种非常方便的算数运算方式, C 编译器喜欢使用
特别是，看起来它的格式看起来像一个有源和目的地的mov指令, 但目的必须是寄存器, 源就是上面寻址格式
它实际上写入寄存器的是地址值
## 其它指令
![[attachments/Pasted image 20250117155425.png]]
注意算数移位和逻辑移位, 对应有符号和无符号
![[attachments/Pasted image 20250117155624.png]] 
![[attachments/Pasted image 20250117155722.png]]
