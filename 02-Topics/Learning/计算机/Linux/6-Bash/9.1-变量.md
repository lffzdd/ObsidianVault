...
![[../attachments/Pasted image 20241008161108.png]]
...
## 环境变量
-  `env`：列出目前的 shell 环境下的所有环境变量与其内容。如果使用`export`也会是一样的内容～ 只不过， `export`还有其他额外的功能![[../attachments/Pasted image 20241008171636.png]]
...
### 几个比较重要的环境变量
- `PS1`：提示字符，可以用 `man bash` 去查询一下 PS1 的相关说明，以理解底下的一些符号意义。![[../attachments/Pasted image 20241008172135.png]]
...
![[../attachments/Pasted image 20241008173606.png]]
...
>底下的例子当中，那个 var 与 str 为变量，我们想要针对 str 是否有设定来决定 var的值喔！ 一般来说， `str:` 代表『str 没设定或为空的字符串时』；至于 `str` 则仅为『没有该变数』。![[../attachments/Pasted image 20241011100736.png]]
