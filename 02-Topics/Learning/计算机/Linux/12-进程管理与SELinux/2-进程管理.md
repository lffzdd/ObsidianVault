![[attachments/Pasted image 20241113160257.png]]
...
# 僵尸进程
![[attachments/Pasted image 20241113161746.png]]
# `top` $P_{711}$
动态进程管理![[attachments/Pasted image 20241113162730.png]]
-  `top -d 2`: 两秒更新一次
- `-p 1321`: 观察 `pid` 为 `1321` 的进程
- `r`: 可以修改优先级![[attachments/Pasted image 20241113162946.png]]
# `pstree`
![[attachments/Pasted image 20241113163024.png]]
`pstree [-A|U] [-up]`
# 讯号 $P_{715}$
![[attachments/Pasted image 20241113171037.png]]
`kill %num` 和 `kill num` 是不同的, 前者是 job, 后者是 `pid`
`kill -SIGHUP $(ps aux | grep 'rsyslogd' | grep -v 'grep'| awk '{print $2}')`
# `killall` $P_{716}$
![[attachments/Pasted image 20241113171329.png]]
```shell
